...
function BallotingType({ applicants }) {
    ...
    const handleSubmit = (e) => {
        ...
        // Filter applicants based on the selected tehsil
        const selectedTehsilApplicants = applicants.filter(
            (applicant) => applicant.tehsil.toLowerCase() === selectedOption.toLowerCase()
        );

        // Set the filtered applicants in state
        setFilteredApplicants(selectedTehsilApplicants);

        // Redirect to the required screen with the filtered applicants using Inertia
        Inertia.post("/path-to-required-screen", {
