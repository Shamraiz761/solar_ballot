...
function Shuffle({ filteredApplicants = "[]" }) {
    ...
    const handleStart = () => {
        ...
        const encodedApplicants = encodeURIComponent(
            JSON.stringify(applicantsArray)
        );
        // alert(se);

        Inertia.get(`/draw?filteredApplicants=${encodedApplicants}`);
        Inertia.get("/draw", {
            filteredApplicants: JSON.stringify(applicantsArray),
        });
        alert(filteredApplicants)
    };
