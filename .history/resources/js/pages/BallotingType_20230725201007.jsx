...
function BallotingType({ applicants }) {
    // Extracting unique tehsil values from applicants array
    const tehsilOptions = [
        ...new Set(applicants.map((applicant) => applicant.tehsil)),
    ];
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-row">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="py-2 relative pl-3 text-xl md:text-2xl text-gray-900 bg-white font-medium border border-gray-600 w-[200px] outline-none"
                            >
                                {selectedOption}
                            </button>
                            {isOpen && (
                                <div className="absolute bottom-[-50px] left-0 z-50 w-[200px] shadow bg-white">
                                    {tehsilOptions.map((option, index) => (
                                        <div
                                            key={index}
                                            className="px-4 py-2 cursor-pointer hover:bg-gray-200 text-lg uppercase font-medium text-gray-800"
                                            onClick={() =>
